...
describe ElasticQueue do
  ...
  before(:all) do
    ...
    Carrot.send(:include, ElasticQueue::Queueable)
    Carrot.send(:queue_attributes, :agent_id, :status, :assigned_to_id, :assigned_at, :expires_at, :follow_up, :priority, :hot, :created_at, :updated_at)
    Cabbage.send(:include, ElasticQueue::Queueable)
    Cabbage.send(:queue_attributes, :status, :license_state)
  end
